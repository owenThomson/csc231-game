...
void Graphics::load_spritesheet(const std::string& filename) {
    ...
    if (sprites.size() == 0) {
        throw std::runtime_error("Could not read any sprites from filename: " +
                                 filename);
    }
}
...
AnimatedSprite Graphics::get_animated_sprite(const std::string& name,
                                             int ticks_per_frame,
                                             bool random_start,
...
void Graphics::draw_rect(const Vec& pixel, const Vec& size, int r, int g, int b,
                         int alpha) {
